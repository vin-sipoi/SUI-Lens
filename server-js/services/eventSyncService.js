...
class EventSyncService {
    ...
    while (hasNextPage) {
      ...
      for (const field of response.data) {
        ...
        if (eventDetail.data && eventDetail.data.content) {
          events.push(this.parseEventData(eventDetail.data.content));
        }
      }
  ...
  parseEventData(content) {
    ...
    const id = typeof rawId === 'object' && rawId !== null ? (rawId.id ?? String(rawId)) : String(rawId || '');
