...
export default function EventDetails({ eventData, onClose }: EventDetailsProps) {
  ...
  const handleClaimPOAP = async () => {
    ...
    setIsMinting(true);
    try {
      // Dynamic POAP minting logic similar to pending approval logic
      const poapEnabled = eventData.poapEnabled ?? true; // Assume true if not specified
      const theme = document.body.classList.contains("theme-light") ? "light" : "dark";

      if (!poapEnabled) {
        alert("POAP minting is not enabled for this event.");
        setIsMinting(false);
        return;
      }

      if (theme !== "light") {
        alert("POAP minting is only available in light theme.");
        setIsMinting(false);
        return;
      }
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            {eventData.requiresApproval && !isRegistered && (
            ...
            {eventData.poapEnabled && isCheckedIn && !hasClaimedPOAP && (
              <Button
                className="w-full bg-yellow-400 hover:bg-yellow-500 text-black py-2 rounded-lg mt-4"
